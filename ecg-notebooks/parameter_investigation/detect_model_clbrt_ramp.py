...
def _setup_argparse():
    ...
    parser.add_argument('--rampstart', action='store', dest='rampstart', type=float, default=1e-7,
                        help='starting ramp if for realization calibration')
    parser.add_argument('--rampmin', action='store', dest='rampmin', type=float, default=1e-18,
                        help='ramp minimum for calibration')
    parser.add_argument('--rampmax', action='store', dest='rampmax', type=float, default=1e-13,
                        help='ramp maximum for calibration')
    ...
    parser.add_argument('-b', '--maxbads', action='store', dest='maxbads', type=int, default=DEF_MAXBADS,
                         help='number of bad ramps to try before expanding the search range')
...
def main():
    ...
    if args.construct or args.detstats:
        ...
        for ii, _data in enumerate(data):
            ...
            hc_bg = _data['hc_bg']
            hc_ss = _data['hc_ss']
            _dsdat = detstats.detect_pspace_model_clbrt_ramp(
                fobs_cents, hc_ss, hc_bg, args.npsrs, args.nskies, args.sigma,
                rampstart=args.rampstart, rampmin=args.rampmin, rampmax=args.rampmax, tol=args.tol, maxbads=args.maxbads,
                thresh=args.thresh, debug=args.debug, red_gamma=args.red_gamma)
            dsdat.append(_dsdat)
        np.savez(save_dets_to_file+'.npz', dsdat=dsdat, sigma=args.sigma, red_gamma=args.red_gamma, npsrs=args.npsrs) # overwrite
    else:
