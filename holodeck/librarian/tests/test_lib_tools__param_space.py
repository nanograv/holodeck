...
def _check_ps_basics(name, pspace_class):
    ...
    total_dims = sum(p.n_params if p.n_params is not None else 1 for p in params)
    assert len(param_names) == nparams, f"{nparams=} and {len(param_names)=}. Expected them to be the same."
    assert total_dims == nparams, f"{nparams=} and {total_dims=}. Expected them to be the same."
    for p in params:
        n = p.n_params
        assert n is None or (isinstance(n, int) and n >= 1), f"Distribution '{p.name}' has invalid n_params={n}"

    # check defaults
    def_params = space.default_params()
    for param in params:
        if isinstance(param.name, (list, tuple)):
            for pname, pdef in zip(param.name, param.default):
                assert def_params[pname] == pdef
        else:
            assert def_params[param.name] == param.default

    np.random.seed(12345)
    idx = 0
    for param in params:
        n = param.n_params if param.n_params is not None else 1
        # Choose random normalized values for all parameters
        norm_vals = np.random.uniform(0.0, 1.0, size=nparams).tolist()
        # Set this specific parameter (which might be multivariate) entirely to None
        norm_vals[idx : idx + n] = [None] * n

        dist_vals = space.normalized_params(norm_vals)

        check_idx = 0
        for p_check in params:
            n_check = p_check.n_params if p_check.n_params is not None else 1
            vv = norm_vals[check_idx : check_idx + n_check]

            if all(v is None for v in vv):
                if n_check == 1:
                    assert dist_vals[p_check.name] == p_check.default, f"default was not set for {p_check.name}"
                else:
                    for pn, pd in zip(p_check.name, p_check.default):
                        assert dist_vals[pn] == pd, f"default was not set for {pn}"
            else:
                if n_check == 1:
                    truth = p_check(vv[0])
                    assert dist_vals[p_check.name] == truth, f"normalized param was not set for {p_check.name}"
                else:
                    truth = p_check([vv])[0]
                    for pn, pt in zip(p_check.name, truth):
                        assert dist_vals[pn] == pt, f"normalized param was not set for {pn}"
            check_idx += n_check
        idx += n
    return
